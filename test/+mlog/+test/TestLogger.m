classdef TestLogger < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function testConvenienceMethods(testCase)
            ...
            msg = "Test convenience methods";


            testCase.writeMsgFun('error', msg);

            testCase.verifyEqual(logObj.LastMessage.Level, mlog.Level.ERROR)
            testCase.verifyNotEmpty(logObj.Messages)
            testCase.verifyMatches(logObj.LastMessage.Text, msg)
            testCase.verifyMatches(logObj.LastMessage.Text, msg)


            testCase.writeMsgFun('warning', msg);

            testCase.verifyEqual(logObj.LastMessage.Level, mlog.Level.WARNING)
            testCase.verifyNotEmpty(logObj.Messages)
            testCase.verifyMatches(logObj.LastMessage.Text, msg)


            testCase.writeMsgFun('info', msg);

            testCase.verifyEqual(logObj.LastMessage.Level, mlog.Level.INFO)
            testCase.verifyNotEmpty(logObj.Messages)
            testCase.verifyMatches(logObj.LastMessage.Text, msg)


            testCase.writeMsgFun('message', msg);

            testCase.verifyEqual(logObj.LastMessage.Level, mlog.Level.MESSAGE)
            testCase.verifyNotEmpty(logObj.Messages)
            testCase.verifyMatches(logObj.LastMessage.Text, msg)


            testCase.writeMsgFun('debug', msg);

            testCase.verifyEqual(logObj.LastMessage.Level, mlog.Level.DEBUG)
            testCase.verifyNotEmpty(logObj.Messages)
            testCase.verifyMatches(logObj.LastMessage.Text, msg)


            testCase.writeMsgFun('detail', msg);

            testCase.verifyEqual(logObj.LastMessage.Level, mlog.Level.DETAIL)
            testCase.verifyNotEmpty(logObj.Messages)
            testCase.verifyMatches(logObj.LastMessage.Text, msg)


            testCase.writeMsgFun('trace', msg);

            testCase.verifyEqual(logObj.LastMessage.Level, mlog.Level.TRACE)
            testCase.verifyNotEmpty(logObj.Messages)
            testCase.verifyMatches(logObj.LastMessage.Text, msg)
